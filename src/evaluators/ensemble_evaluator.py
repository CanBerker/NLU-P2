...
class EnsembleEvaluator(object):
    @staticmethod
    def validation_error(strategy: Strategy, training_data: np.ndarray,
                validation_data: np.ndarray, augmented_data: np.ndarray) -> float:
                
        val_stories, val_labels = np.split(validation_data, [-1], axis=1 )
        val_labels = np.squeeze(val_labels)
        
        strategy.fit(training_data, validation_data, augmented_data)
